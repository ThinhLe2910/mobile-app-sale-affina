...
class BasePopup: UIView {
    ...
    static let nib = "BasePopup"
    
    class func instanceFromNib() -> BasePopup {
        return UINib(nibName: self.nib, bundle: nil).instantiate(withOwner: nil, options: nil)[0] as! BasePopup
    }
    ...
    func setPopupData(icon: UIImage?, title: String, desc: NSMutableAttributedString, okTitle: String, cancelTitle: String) {
        imageView.image = icon
        titleLabel.text = title
